document.addEventListener("DOMContentLoaded", async () => {
    ...
    // ------------------------
    // 側邊分頁切換 (移除重複的 initTabSwitch 定義，只保留一次)
    // ------------------------
    ...
    initTabSwitch(".order-tab", ".order-tab-content");
    ...
    async function loadProfile() {
        ...
        document.getElementById("save-profile-btn").addEventListener("click", async () => {
            const { value: full_name } = document.getElementById("profile-full_name");
            const { value: phone } = document.getElementById("profile-phone");
            const { value: address } = document.getElementById("profile-address");
            const { value: gender } = document.getElementById("profile-gender");
    ...
    async function loadPets() {
        // 優化: 使用 limit(1).maybeSingle() 限制只抓取一筆資料，提高效率
    ...
    document.getElementById("add-pet-btn")?.addEventListener("click", async e => {
        ...
        // 優化: 檢查寵物數量時，使用 count: "exact", head: true
        const { count: petCount } = await supabaseClient
            .from("pets")
            .select("id", { count: "exact", head: true })
            .eq("user_id", currentUser.id);

        if (petCount > 0) return alert("每位會員只能新增一隻寵物喔！");
